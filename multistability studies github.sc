...
(
{
	...
	snd = MoogFF.ar(snd, LFNoise2.kr(0.1).linexp(-1, 1, 500, 16000), 0);
	snd = snd ! 2;
	snd;
}.play(fadeTime: 0);
)

//////Mark Fell multistability study
(
SynthDef(\fm, {
	arg out=0, pan=0, amp=1, freq=111, atk=0.001, rel=0.2,
	...
	cascade_0 = SinOsc.ar(freq, SinOsc.ar(freq * \ratio3.kr(1.618), pi * 0.5).range(0, mInd3) ) * \amp_3.kr(0.5);
	cascade_0 = SinOsc.ar(freq, SinOsc.ar(freq * \ratio2.kr(1.618), pi * 0.5).range(0, mInd2) + cascade_0.range(0,mInd2)) * \amp_2.kr(0.5);
	cascade_0 = SinOsc.ar(freq, SinOsc.ar(freq * \ratio1.kr(1.618), pi * 0.5).range(0, mInd1) + cascade_0.range(0,mInd1)) * \amp_1.kr(0.5);

	cascade_1 = SinOsc.ar(freq, SinOsc.ar(freq * \ratio6.kr(1.618), pi * 0.5).range(0, mInd6) + LocalIn.ar(1).range(0, mInd6)) * \amp_6.kr(0.5);
	cascade_1 = SinOsc.ar(freq, SinOsc.ar(freq * \ratio5.kr(1.618), pi * 0.5).range(0, mInd5) + cascade_1.range(0,mInd5)) * \amp_5.kr(0.5);
	cascade_1 = SinOsc.ar(freq, SinOsc.ar(freq * \ratio4.kr(1.618), pi * 0.5).range(0, mInd4) + cascade_1.range(0,mInd4)) * \amp_4.kr(0.5);
...
~viewSet = {
	arg set;
	var window = Window.closeAll.new("Multistability Sequencer", Rect.new(300, 300, 1200, 400));
